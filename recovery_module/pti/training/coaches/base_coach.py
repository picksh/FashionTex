...
from utils.models_utils import load_pre_model, toogle_grad, load_old_G,load_generator
import numpy as np
...
class BaseCoach:
    ...
    def continue_training(self):
        # Initialize networks
        self.G = load_pre_model()
        toogle_grad(self.G, True)

        self.original_G = load_pre_model()
    ...
    def load_inversions_edit(self, w_path_dir, image_name):
        #w_path=os.path.join(w_path_dir,image_name,'0.npy')
        w_path=os.path.join(w_path_dir,f'{image_name}.npy')
    ...
    def forward(self, w):
        ...
        #generated_images,_ = self.G([w], input_is_latent=True, randomize_noise=True, truncation=1)
