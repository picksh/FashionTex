...
class DenseCLIP(BaseSegmentor):
    ...
    def __init__(self, **args):
        super(DenseCLIP, self).__init__(args["init_cfg"])
        pretrained=args["pretrained"]
        backbone=args["backbone"]
        text_encoder=args["text_encoder"]
        context_length=args["context_length"]
        context_decoder=args["context_decoder"]
        score_concat_index=args["score_concat_index"]
        context_feature=args["context_feature"]
        text_head=args["text_head"]
        tau=args["tau"]
        neck=args["neck"]
        decode_head=args["decode_head"]
        auxiliary_head=args["auxiliary_head"]
        identity_head=args["identity_head"]
        ...
        self.train_cfg = args["train_cfg"]
        self.test_cfg = args["test_cfg"]

        self.texts = torch.cat([tokenize(c, context_length=self.context_length) for c in args["class_names"]])
        self.num_classes = len(self.texts)


        context_length = self.text_encoder.context_length - self.context_length
        self.contexts = nn.Parameter(torch.randn(1, context_length, args["token_embed_dim"]))
        nn.init.trunc_normal_(self.contexts)
        self.gamma = nn.Parameter(torch.ones(args["text_dim"]) * 1e-4)
    ...
    def after_extract_feat(self, x):
        x_orig = list(x[0:4])
        global_feat, visual_embeddings = x[4] # z
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        #print(text_embeddings.shape) # 4 24 512
        #print(len(x_orig))
        #print(x_orig[0].shape) # 4 768 160 160
